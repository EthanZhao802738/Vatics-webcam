 ...
 * \author djhow.tu@vatics.com ubuffer modifier
 *
 *******************************************************************************
 */
...
/*! 
    ...
    The struct of TAG_MP4V_EXTENIFO user data item:<br>
...
/*! \page videoinubuf Video in ubuffer
    \section h265inubuf H265 in ubuffer
    \par Ubuffer conf for h265
	Format of ubuffer conf for h265 is described in the diagram below.<br>
    Each header field is decribed in the followings:
    \li \c dwSize: size of this ubuffer conf.
    \li \c dwDataType: must be FOURCC_CONF.
    \li \c dwFollowingDataType: must be FOURCC_H265.
    \li \c dwTrackID: track id of this h265 stream.
    \li \c dwSec,\c dwUSEc,\c dwSeqNo,\c dwIsSync,\c dwCompositionOffset,\c bIsBoundary: meanless here.
    \li \c dwUserDataSize: user data size in this ubuffer conf.
    \li \c dwWidth: width of the video.
    \li \c dwHeight: height of the video.
    \li \c abyProfileLevel: 3 bytes long and defined in RFC 3984.
    \li \c VPS \c and \c SPS \c and \c PPS \c special \c format: variable length in special format to carry VPS and SPS and PPS, see the format definition in the following paragraph.
<pre>
      1byte   1byte   1byte   1byte
    |<----->|<----->|<----->|<----->| 
    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    \par VPS and SPS and PPS special format in ubuffer conf for h265
    SPS stands for sequence parameter set, PPS stands for picture parameter set, VPS stands for Video Parameter Set.<br>
    It's a 2 bytes length field followed by one VPS or SPS or PPS NAL.<br>
    Ubuffer conf for h265 \em must contains at least one VPS and at least one SPS and at least one PPS.<br>
    There's no limitation of how may VPS/SPS/PPSs can be carried here;<br>
    Unless the physic buffer isn't big enough, this should be handled by programmer.<br>
    The value in the length field means how many bytes of the following VPS or SPS or PPS.<br>
    The length field is host byte order (in ARM, it is little endian).
...
<pre>
    ...
    |<--------->|<--------------------------------->|<--------->|
    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    | length(i) |        NAL of VPS or SPS or PPS   |length(i+1)| .........
    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
</pre>
    \par Ubuffer to carry h265 bitstream
    Each header field is decribed in the followings:
    \li \c dwSize: size of this ubuffer.
    \li \c dwDataType: must be FOURCC_H265.
    \li \c dwFollowingDataType: meanless here.
    \li \c dwTrackID: track id of this h265 stream.
    \li \c dwSec: second portion of timestamp.
    \li \c dwUSEc: micro second portion of timestamp.
    ...
    \li \c dwIsSync: if carrying IDR frame set to TRUE, otherwise set to FALSE.
    \li \c dwCompositionOffset: meanless here.
    \li \c bIsBoundary: if carrying last portion of one h265 frame set to TRUE, otherwise set to FALSE.
    \li \c dwUserDataSize: user data size in this ubuffer conf.
    \par
    Store multiple NALs in ubuffer media data portion.<br>
    One NAL can't be segmented into multiple ubuffers.<br>
...
/*! \brief UBuffer of Configuration for H.265
    \see u_buffer

  UBuffer configuration for H.265. The following payload of this buffer is structured as 
  a series of the length of NAL unit, video parameter set (VPS) or picture parameter set (PPS) or sequence parameter set (SPS), 
  and the NAL unit itself. The length of the fields that describe the length of NAL unit is 2 byte
  and is in host byte order (little-endian). 
 <pre> 
  sizeof(TUBufferConfH265) bytes    2 bytes   m bytes, m < 65536
|<------------------------------>|<-------->|<------------------>|
+--------------------------------+----------+--------------------+-------+-----------
|    TUBufferConfH264            |Length (m)|  VPS or SPS or PPS |Length |................
+--------------------------------+----------+--------------------+-------+-----------
 </pre>
 
	\note 
	The payload of H.265 UBuffer, which means that u_buffer::dwDataType is FOURCC_H265, 
	is formated as a series of the length of NAL unit, and the NAL unit itself. 
	The length of the fields that describe the length of NAL unit is 4 byte
	and is in host byte order (little-endian). 
 <pre> 
 ...
 */
typedef struct u_buffer_conf_h265
{
    /*! see define#UBUFFER_PROTOTYPE */
	UBUFFER_PROTOTYPE;
	/* specific */
