 ...
 *				This demo demostrates client-side two way audio using TCP socket and Atk.
 * @author		Max Chang
 * @date		2016/2/25
 *
 */
...
#define APP_NAME		"audio_client"
twa_ctx audio_client_mmap_config;
...
static void sig_kill(int signo)
{
	...
	switch(signo)
	{
		case SIGTERM:
		case SIGINT:
			twa_ctx_stop(&audio_client_mmap_config);
			break;
		default:
			break;
	}
...
int main(int argc, char* argv[])
{
    ...
    twa_ctx_init(&audio_client_mmap_config, CLIENT_SITE);

	ret_val = twa_ctx_load_prog_args(argc,argv,&audio_client_mmap_config);
	...
	ret_val = twa_ctx_network_init(&audio_client_mmap_config);
	if(ret_val == -1)
		goto main_end;
	...
	twa_ctx_audio_init(&audio_client_mmap_config);
	if(ret_val == -1)
		goto main_end;

	twa_ctx_start(&audio_client_mmap_config);
	

main_end:
	twa_ctx_audio_release(&audio_client_mmap_config);
	twa_ctx_network_release(&audio_client_mmap_config);
	return ret_val;
}
